...
export async function POST(req: Request) {
  ...
  const systemPrompt = `You are acting as an assistant to a customer service agent who is interacting with a customer. Your goal is to assist the customer service agent by providing essential next steps and relevant information to satisfy the customer's request. When responding to the agent, aim to provide concise and helpful guidance while maintaining a professional tone.

  To help you address the customer's question, we have retrieved the following information for you. It may or may not be relevant (we are using a RAG pipeline to retrieve this information):
  ${
    isRagWorking
      ? `${retrievedContext}`
  ...
  Please provide responses that only use the information you have been given. If no information is available or if the information is not relevant for answering the question, you can suggest that the customer service agent escalates the issue or redirects the customer to another department for further assistance.
  ...
  You are assisting the customer service agent in resolving the customer's issue by providing essential next steps and relevant information. Your goal is to help the agent satisfy the customer's request efficiently.
